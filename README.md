...
[
  {
    ...
    "settings": {
      ...
      "tradeMode": {},         // 交易模式: both 或 long_only
      "shortSignalState": {}   // 空头信号状态: normal 或 ignored_temporarily
    }
  },
  {
    "id": "user2",
...
*   **功能**：
    *   **启用/禁用**：控制特定币种是否参与交易。
    *   **交易模式**：支持双向（both）和只做多（long_only）。
    *   **空头信号状态**：支持正常（normal）和临时忽略（ignored_temporarily）。
    *   **修改金额**：调整基础开仓金额或全局倍数，实时生效。

### Telegram 命令控制
默认频道支持以下命令（单用户默认作用于当前机器人）：

*   `/状态 [交易对]`：查看参数状态
*   `/模式 交易对 双向|只做多`：设置交易模式
*   `/空头 交易对 正常|临时忽略`：设置空头信号状态
*   `/启用 交易对 开|关`：开关交易
*   `/恢复空头 [交易对|ALL]`：恢复空头信号状态
